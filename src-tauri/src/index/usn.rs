...
impl UsnMonitor {
    ...
    pub async fn start_monitoring(&mut self) -> Result<()> {
        ...
        loop {
            if self.stop_flag.load(Ordering::Relaxed) {
                log::info!("USN monitoring for drive {} stopped", self.drive_letter);
                break;
            }
    ...
    async fn handle_usn_record(
        ...
        if record.reason & USN_REASON_FILE_CREATE != 0 {
            ...
            index.update_entry(self.drive_letter, record.file_reference_number, entry);
        }
